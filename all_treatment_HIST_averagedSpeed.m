...
common_bins = linspace(-120, 120, 100); % from -120 to 120 μm/s, 100 bins

% we interpolate the date points for each group to find common bins:

% control group:
freq_ctr1 = interp1(ctr004speed{:,1}, ctr004speed{:,2}, common_bins, 'linear', 0);
freq_ctr2 = interp1(ctr007speed{:,1}, ctr007speed{:,2}, common_bins, 'linear', 0);
ctr_avg_freq = (freq_ctr1 + freq_ctr2) / 2;
% octopamine group:
freq_oa1 = interp1(oa008speed{:,1}, oa008speed{:,2}, common_bins, 'linear', 0);
freq_oa2 = interp1(oa009speed{:,1}, oa009speed{:,2}, common_bins, 'linear', 0);
oa_avg_freq = (freq_oa1 + freq_oa2) / 2;
% serotonin group:
freq_ser1 = interp1(ser010speed{:,1}, ser010speed{:,2}, common_bins, 'linear', 0);
freq_ser2 = interp1(ser011speed{:,1}, ser011speed{:,2}, common_bins, 'linear', 0);
